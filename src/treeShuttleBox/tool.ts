...
export const handleFindParent = (arr: Array<item>, leftArr: Array<item>) => {
  const newArr: Array<item> = []
  arr.map(i => {
    if (i.parentId && arr.find(j => j.parentId !== i.key)) {
      const parent: item = leftArr.find(k => k.key === i.parentId) as item
      newArr.push(parent)
    }
  })

  const deepDewArr = JSON.parse(JSON.stringify([...arr, ...newArr]))

  const result = deepDewArr.reduce((acc: Array<item>, curr: item) => {
