...
import { searchAllParents, reorganizeTree, traverseTreeData, handleFindParent } from "./tool";
  ...
  const [handleClickToRight, handleClickToLeft] = useMemo(() => {
    ...
    const handleClickToLeft = () => {
      ...
      if (checkListRight.length === readOnlyAllKeyRight.length) {
        ...
        // 重组右侧树形数据
        const treeData = reorganizeTree({
          treeArr: handleFindParent( selectNode,flatArray),
        });
        ...
        console.log(treeData, 'treeDatatreeDatatreeDatatreeData22');
        // 保存左侧选择的key
